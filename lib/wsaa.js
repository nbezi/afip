...
class Wsaa extends Wsafip {
			...
			if (err) callback(err);
				...
				if (!err) {
					this._setCredentials({
						...
						cuit: (/<destination>.*CUIT (\d+),.*<\/destination>/g).exec(result.loginCmsReturn)[1]
					});
				}
	...
	createCMS(cb) {
        ...
        fs.writeFile(tarFilename + '.xml', this.createTRA(), (err) => {
	        ...
	        /*command = util.format(
				'cat %s | openssl smime -sign -text -signer %s -inkey %s -outform PEM -nodetach',
				tarFilename + '.xml', this.getCertFilename(), this.getKeyFilename()
			);*/
